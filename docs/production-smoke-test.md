...
The wallet deployment must not call transaction broadcast endpoints during Phase 4.4.
...
9. Press `Details` on one transaction row and confirm a transaction detail card appears.
10. Confirm the detail request is only:

```text
GET /api/wallet/tx/{txid}
```

11. Enter an invalid PEPEW address and confirm the error is user-facing, not a raw internal code.
12. In DevTools Network and Console, search for the forbidden terms above.
